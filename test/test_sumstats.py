...
from numpy.testing import assert_array_equal, assert_array_almost_equal, assert_allclose, assert_almost_equal
import pytest
import os
...
class Test_sumstats_functions(unittest.TestCase):

    def test_check_condnum(self):
        x = np.ones((2, 2))
        x[1, 1] += 1e-5
        args.invert_anyway = False
        pytest.raises(ValueError, s._check_ld_condnum, args, log, x)
...
class Test_RG_Statistical(unittest.TestCase):
...
class Test_Estimate(unittest.TestCase):
    ...
    def test_h2_M(self):  # check --M works
        ...
        args.M = '1,2'
        pytest.raises(ValueError, s.estimate_h2, args, log)
        args.M = 'foo_bar'
        pytest.raises(ValueError, s.estimate_h2, args, log)
    ...
    def test_rg_M(self):
        ...
        args.M = '1,2'
        pytest.raises(ValueError, s.estimate_rg, args, log)
        args.M = 'foo_bar'
        pytest.raises(ValueError, s.estimate_rg, args, log)
